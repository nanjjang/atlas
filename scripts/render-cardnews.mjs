...
const outputDir = path.join(root, "media/cardnews/codraw-ko");
const backgroundPath = path.join(outputDir, "codraw-graph-background.png");
...
function footer(page) {
  ...
  return `${dots}<text x="54" y="1299" fill="#64748b" font-size="21" font-family="Noto Sans CJK KR, sans-serif">github.com/nanjjang/codraw</text><text x="1026" y="1299" fill="#94a3b8" font-size="20" font-weight="700" text-anchor="end" font-family="Noto Sans CJK KR, sans-serif">CODRAW</text>`;
}
...
async function renderCover() {
  ...
  const overlay = svg(`
    ...
    ${textLines(["Codraw · VS Code 안에서 코드 구조를 탐색하는 익스텐션"], 54, 1040, 27, 500, 1.2, "#cbd5e1")}
    ${footer(1)}
  `, defs);
...
async function renderEnd() {
  const defs = `<linearGradient id="codraw" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#60a5fa"/><stop offset="0.55" stop-color="#38bdf8"/><stop offset="1" stop-color="#5eead4"/></linearGradient>`;
  const overlay = svg(`
    <text x="540" y="560" fill="url(#codraw)" font-size="172" font-weight="900" letter-spacing="10" text-anchor="middle" font-family="Noto Sans CJK KR, sans-serif">CODRAW</text>
    ...
    <text x="540" y="1112" fill="#94a3b8" font-size="24" text-anchor="middle" font-family="Noto Sans CJK KR, sans-serif">github.com/nanjjang/codraw</text>
    ${footer(6)}
  `, defs);
...
await renderFeature({
  ...
  screenshot: path.join(root, "media/screenshots/codraw-files.png"),
});
await renderFeature({
  page: 3,
  ...
  screenshot: path.join(root, "media/screenshots/codraw-flow.png"),
});
await renderFeature({
  page: 4,
  eyebrow: "DATA MODEL",
  ...
  screenshot: path.join(root, "media/screenshots/codraw-data-model.png"),
});
await renderFeature({
  page: 5,
  ...
  screenshot: path.join(root, "media/screenshots/codraw-interfaces-polyglot.png"),
});
await renderEnd();
